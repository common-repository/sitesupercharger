...
class Ssc_SuperCharger {
	...
	protected static $ssc_plugin_version = '5.3.3';
	...
	/**
	 * Check HTML tag for replace keywords to link in content
	 *
	 * @since 5.3.3
	 *
	 *
	 * @param string $tag The tag needs to be search.
	 * @param array $match The match array of the keyword.
	 * @param string $content The content of the page or post.
	 * @param boolean $linkIt The variable of decide to link tag or not.
	 * @return boolean Return wether link the tag or not.
	 */

	private static function interlinkingTag( $tag, $match, $content, $linkIt ){
		$tag_open = strrpos( substr( $content, 0, $match[1] ), "<".$tag );
		if( $tag_open !== FALSE ) {
			//find first tag after the previously found tag
			$tag_close = strpos( $content, "/".$tag.">", $tag_open );
	...
	public static function interlinking( $content ) {
		...
		foreach($matches[0] AS $match){
			...
			$linkIt = self::interlinkingTag( 'a', $match, $content, $linkIt );

			//find last <H1> tag before the keyword
			$linkIt = self::interlinkingTag( 'h1', $match, $content, $linkIt );

			//find last <H2> tag before the keyword
			$linkIt = self::interlinkingTag( 'h2', $match, $content, $linkIt );

			//find last <H3> tag before the keyword
			$linkIt = self::interlinkingTag( 'h3', $match, $content, $linkIt );
	...
	private static function initPhoneSwap() {
		...
		if(!isset($_COOKIE["ssc_phoneSwap"])){
			...
			if (!empty($http_referer)) { // Check if $http_referer is not empty before using strpos
